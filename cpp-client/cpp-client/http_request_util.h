...
namespace client
{
	...
	response send_request(const std::string& serverIP, unsigned short int serverPort, request<Request_Class>& request)
	{
		...
		request.header.payload_size = request.serialized_payload_data.empty() ? sizeof(request.payload) : request.serialized_payload_data.size(); // send payload_custom_size if sett
		...
		if (send(client_socket, request.serialized_payload_data.empty() ? reinterpret_cast<char*>(&request.payload) : request.serialized_payload_data.c_str(), request.header.payload_size, 0) == SOCKET_ERROR) {
